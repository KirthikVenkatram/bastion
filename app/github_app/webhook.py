...
async def _run_pipeline_background(
    repo_full_name: str, installation_id: int, pusher_email: str | None
) -> None:
    """Run the pipeline while preserving webhook delivery on background failures."""
    try:
        await run_pipeline(repo_full_name, installation_id, pusher_email)
    except Exception:
...
async def handle_webhook(request: Request, background_tasks: BackgroundTasks) -> dict[str, str]:
    ...
    if event == "push":
        ...
        pusher_email = pusher.get("email") if isinstance(pusher, dict) else None
        if not isinstance(pusher_email, str):
            pusher_email = None
